...
- `orbit :: Vector{RVec{D}}`: the δᵢ elements of the orbit generated by δ. The elements are
  sorted by [`sort_orbit_by_sign_pairs!`](@ref) and so need not include `representative` as
  their first element
...
function _getindex(
    ...
    # print the contributing terms by ascending canonical index (zₘ before z̄ₘ) rather than
    # in orbit order, which can interleave ±δ partners (e.g., `z₃+z̄₁+z̄₂`)
    δs = tbb.h_orbit.orbit
    nonzero_idxs = (n for n in eachindex(MⁱʲtC)
                    if abs(MⁱʲtC[n]) ≥ SPARSIFICATION_ATOL_DEFAULT)
    contributions = map(nonzero_idxs) do n
...
Crystalline.dim(::AbstractParameterizedTightBindingModel{D}) where D = D
orbital_count(aptbm::AbstractParameterizedTightBindingModel) = orbital_count(aptbm.tbm)
