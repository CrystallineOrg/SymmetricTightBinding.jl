...
function hop_plot(hop::HoppingOrbit{D}) where {D}
    ...
    ending_atoms =
        reduce(vcat, [[P(constant(h[2] + h[3])) for h in hs] for hs in hop.hoppings])
    ...
    arrows!(
        ax,
        starting_atoms,
        V.(ending_atoms - starting_atoms) .* 0.8;
        color = :gray,
        linewidth = 0.025,
        arrowsize = D == 3 ? V(0.075, 0.075, 0.1) : V(0.075, 0.1),
        label = "Bonds",
    )
