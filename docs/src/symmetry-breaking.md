...
!!! warning "Pass the hopping range, and a complete model"
    `subduced_complement` should be given the same hopping range `Rs` that `tbm` was built over, with `tbm` the complete model over that range:
    ```julia
    Rs   = [[0,0], [1,0]]
    tbm  = tb_hamiltonian(cbr, Rs)
    Δtbm = subduced_complement(tbm, Rs, 6)
    ```
    Only then are the returned terms exactly those that the subgroup allows and the original group forbids. Restricting `tbm` to a subset of its terms, or omitting `Rs`, instead gives the complement relative to whatever was passed: terms that the original group allows perfectly well can then come back as "new".

    The walkthrough on this page restricts `tbm` to its four simplest terms for brevity, and its output should be read accordingly.
